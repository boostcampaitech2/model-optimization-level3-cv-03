...
expr_name= 'NAS_smallEP_3crit'

def sigmoid(x: float)->float:
    return 1 / (1 + 1/(np.e ** x))

def get_score(my_time, my_f1):
    return 0.5 * my_time / 60.0 + sigmoid(20* (0.7-my_f1))

def search_hyperparam(trial: optuna.trial.Trial) -> Dict[str, Any]:
    """Search hyperparam from user-specified search space."""
    # epochs = trial.suggest_int("epochs", low=20, high=20, step=50)
    img_size = trial.suggest_categorical("img_size", [64, 96, 112, 168, 224])
    # n_select = trial.suggest_int("n_select", low=0, high=6, step=2)
    # batch_size = trial.suggest_int("batch_size", low=512, high=512, step=0)
    max_lr = trial.suggest_float(name='max_lr', low=1e-4, high=0.1)
    return {
        "EPOCHS": EPOCH,
        "IMG_SIZE": img_size,
        "MAX_LR": max_lr,
...
def search_model(trial: optuna.trial.Trial) -> List[Any]:
    ...
    if m4 == "Conv":
        # Conv args: [out_channel, kernel_size, stride, padding, groups, activation]
        m4_out_channel = trial.suggest_int("m4/out_channels", low=16, high=64, step=16)
    ...
    if m7 == "Conv":
        ...
        m7_out_channel = trial.suggest_int(
            "m7/out_channels", low=64, high=256, step=128
        )
...
def objective(trial: optuna.trial.Trial, device) -> Tuple[float, int, float]:
    ...
    data_config["DATASET"] = "TACO_light"
    ...
    _,_, my_time = trainer.train(train_loader, EPOCH, val_dataloader=val_loader)
    loss, f1_score, acc_percent = trainer.test(model, test_dataloader=val_loader)
    params_nums = count_model_params(model)

    model_info(model, verbose=True)
    # return get_score(my_time, f1_score)
    # return 1.9-get_score(my_time, f1_score), params_nums, mean_time
